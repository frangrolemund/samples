...
- (void)viewDidLoad
{
    ...
    [self setSourceColor:[UIColor colorWithHue:0.55f saturation:1.0f brightness:0.75f alpha:1.0f]];
...
-(IBAction)doChangeSlider:(id)sender
{
    ...
    if (sender == slColorFax) {
        ...
        if (index == 3) {
            ...
            // - synchronization:
            //   my theory is that the distance between hues is identical when we get the right kind of result.  In order to
            //   ensure that, I'll manually update the value for hue B also at the same time to be the inverse of the
            //   delta.
            UISlider *slPrimaryHue = (UISlider *) [self.view viewWithTag:101];
            CGFloat delta = slPrimaryHue.value - hueA;
...
-(IBAction)doDumpData:(id)sender
{
    ...
    for (NSUInteger i = 0; i < [arr count]; i++) {
        ...
        if (doDumpNow) {
            ...
            NSString *sFile = [NSString stringWithFormat:@"colordata-%d.csv", (int) (curSat * 1000)];
    ...
    NSString *sMsg = [NSString stringWithFormat:@"%d item%s output to CSV.", (int) [mdSnapData count], [mdSnapData count] > 1 ? "s were" : " was"];
         ...
         usingResolution:(pscf_colorfax_resolution_t) res
{
    ...
    CGFloat delta = PSCF_COLORFAX_MIN_DELTA + (PSCF_COLORFAX_TICK * (CGFloat) (res + 1));
    delta *= (1.0f - pctOfAdj);
    *hA  = h - delta;
    if (*hA < 0.0f) {
        *hA += 1.0f;
...
-(void) updateNumberField:(int) field withValue:(CGFloat) value
{
    ...
    if (field == 1 || field == 2) {
        ...
        UISlider *slSubHue = (UISlider *) [self.view viewWithTag:104];
        if ([self isColorFaxEnabled]) {
            newAlpha = 1.0f;
            slSubHue.enabled = NO;
        }
        else {
            slSubHue.enabled = YES;
        }
...
{
    NSURL *u = [[NSFileManager defaultManager] URLForDirectory:NSDocumentDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:YES error:nil];
    return [u URLByAppendingPathComponent:@"colorsplits.kar"];
}
...
-(void) dumpColorFaxConstants
{
    ...
    NSURL *u = [[NSFileManager defaultManager] URLForDirectory:NSDocumentDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:YES error:nil];
    u = [u URLByAppendingPathComponent:@"colorfax.csv"];
    ...
    u = [u URLByAppendingPathComponent:@"colorfax-code.txt"];
    if (![sColorFaxData writeToURL:u atomically:YES encoding:NSASCIIStringEncoding error:nil]) {
